function [label,model,llh,llh_BIC,p] = EM_PMoEP_mu(X,param,p,lambda)
...
if length(p)~=length(find(p==2))
    [R, llh(t),llh_BIC] = expectation_mu(X,model, p, lambda);
else
...
while ~converged && t < maxiter
    ...
    if length(p)~=length(find(p==2))
        [model,~,p,echo] = maximizationModel_mu(X,R,model.eta,model.mu,p,lambda);
    else
        [model,R,echo,k] = maximizationModel_PMoG_mu(X,R,lambda);
        p = 2*ones(1,k);
    end
