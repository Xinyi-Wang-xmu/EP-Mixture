function [label,model,llh,llh_BIC,p] = NEM_PMoEP(X,param,p)
...
if length(p)~=length(find(p==2))
    [R, llh(t),llh_BIC] = Nexpectation(X,model, p, lambda);
else
    [R, llh(t),llh_BIC] = Nexpectation_PMoG(X,model,lambda);
end
...
while ~converged && t < maxiter
    ...
    if length(p)~=length(find(p==2))
        [model,~,p] = NmaximizationModel(X,R,p,lambda);
    else
        [model,R,k] = NmaximizationModel_PMoG(X,R,lambda);
        p = 2*ones(1,k);
    end
